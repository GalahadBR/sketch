...
S3_ENDPOINT_URL = os.getenv(
    'S3_ENDPOINT_URL', 'http://vini-sketch-legacy-s3.s3-website.us-east-2.amazonaws.com')
...
def create_s3_object(s3_conn, bucket, path):
    try:
        if AVATAR_FILE:
            s3_conn.Bucket(bucket).upload_file(
                Key=f"{path}", Filename=AVATAR_FILE)
        else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='This script seeds the database and s3 bucket with the number of legacy avatars passed as a first argument.')
    parser.add_argument('number_of_avatars', type=int,
                        help='Number of legacy avatars to create')
